/*
 * s3_paul.c -- A* DAG search to create N-best lists
 *
 * HISTORY
 *
 * P. Deleglise at LIUM, University of Le Mans, France
 *      Started, copying from astar.c, written by M K Ravishankar
 *      at Carnegie Mellon University on 27-Feb-1998
 */
...
{int count;
	...
	if ((hlist->dagnode != node) || (hlist->histhash != hval))
	    continue;
	count=dict_filler_word(astar->dict,node->wid)? astar->ngram-1 : astar->ngram-2;    /* 3 :2;*/
...
static void ppath_insert (astar_t * astar,ppath_t *top, daglink_t *l, int32 pscr, int32 tscr, int32 lscr,
  ...
  loop=lmhist;
  for( loop=lmhist ; loop  && compte<astar->ngram-1; loop=loop->lmhist,compte++) 
    w[compte]= dict_basewid(astar->dict,loop->dagnode->wid);
  for( ; compte<astar->ngram-1; compte++)
    w[compte]=BAD_S3WID;

  h =(long) l->node;
  h = (h >> 5) | (h << 27);
  h=w[0]+ 37*w[1]+  809*w[2];
  if (astar->ngram>4) h+= w[3]*22091;
  for (compte=4 ; compte <astar->ngram-1 ;compte++) {
    h=(h>>5) | (h<<27);
    h += w[compte];
    h %= HISTHASH_MOD;
  }
  ...
  if (fp) fprintf(fp,"J=%d\tS=%d\tE=%d\ta=%f\tl=%f\n",edgeCount++,top->num,
	  num,logs3_to_log(l->ascr),logs3_to_log(lm_rawscore (astar->lm,lscr)));
...
astar_init(dag_t *dag, dict_t *dict, lm_t *lm, fillpen_t *fpen, float64 beam, float64 lwf)
{
    ...
    astar->ngram = cmd_ln_int32("-ngram");
    astar->n_ppath = 0;
		   ...
		   dict_t *dict, lm_t *lm, fillpen_t *fpen ,FILE * ctmfp)
{
    ...
    s3wid_t bwfinal; /*bwm1,bw0, bw1, bw2;*/
    s3wid_t bwid[20];
    int32 ispipe;
    ...
    /*int debutGraphe; */
    int k;
    bzero(lwid,20*sizeof(s3lmwid32_t)); /* set for lru cache in case of odd order */
...
sscanf(uttid,"%[^-]-%f",astar->showName,&(astar->startTime));
	...
	if (top->pruned)
	    continue;
	/* some doubt about final.ascr because final is after end ????? */
...
/* 	    bw0 = bw1 = BAD_S3WID;*\/ */
	...
	for (l = d->succlist; l; l = l->next) {
	  assert (l->node->reachable && (1 || ! l->bypass)); /* I don't use bypass */

	    /* Obtain LM score for link */
	    bwfinal = dict_basewid (dict, l->node->wid);
	    if (dict_filler_word (dict,bwfinal)) lscr= fillpen(fpen, bwfinal) ;
	    else { 
	      lwid[astar->ngram-1] =lm->dict2lmwid[bwfinal];
	      lscr=lwf*lireLRU(lwid);
	      if ( lscr ==10000) {
		pscr = top->pscr + l->ascr + lscr+dict_proba(dict,l->node->wid);
		tscr = pscr + l->hscr;
		lscr=lwf * lm_ng_score(lm, astar->ngram, lwid, bwfinal);
		pp=top;
                for (k=4 ;k>=0 && pp!=NULL; k--, pp=pp->lmhist) 
		  printf (" k :%d id:%d  sf=%5d, %s \t",k,pp->dagnode->seqid,pp->dagnode->sf,dict_wordstr(dict,pp->dagnode->wid));
		printf("\n");
		printf ("%d pscr= %11d, hscr= %11d, tscr= %11d, lscr=%7d, pron=%6f sf= %5d, %s%s %d leBest: %d\n",l->node->seqid,
			pscr, l->hscr, tscr,lscr,dict_proba(dict,l->node->wid), l->node->sf, dict_wordstr(dict, l->node->wid),
			(tscr-astar->beam >= besttscr) ? "" : " (pruned)",besttscr,dag->bestScr);
	    ...
	    if (tscr -astar->beam >= besttscr) {
	      ppath_insert (astar,top, l, pscr, tscr, lscr+dict_proba(dict,l->node->wid) ,fp);
		if (astar->n_ppath > astar->maxppath) {
		    E_ERROR("%s: Max PPATH limit (%d) exceeded\n", uttid, astar->maxppath);
		    break;
		}
/* 		if (0) { */
/* 		  if( tscr>besttscr && debutGraphe&&dict_filler_word(dict,top->dagnode->wid)&& */
/* 		      !dict_filler_word(dict,l->node->wid)) { */
/* 		    fprintf(stderr,"-----> %s %s %s \n",dict_wordstr(dict,top->dagnode->wid), */
/* 			    dict_wordstr(dict,l->node->wid), */
/* 			    dict_wordstr(dict,bw1)); */
/* 		    besttscr=tscr; */
/* 		  } */
		  
/* 		  /\* code to verify absolutely *\/ */
/* 		  if (tscr > besttscr&& !dict_filler_word(dict, l->node->wid)&& !dict_filler_word(dict,top->dagnode->wid) ) */
/* 		    besttscr = tscr; */
/* 		} */
    ...
    if (fp && fp!=stdout) fclose_comp (fp, ispipe);
